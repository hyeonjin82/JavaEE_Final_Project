...
<script>
 function setHiddenVal(){
	 var e = document.getElementById("tablePage");
	 var strUser = e.options[e.selectedIndex].value;
	 document.getElementById("hidden").value = strUser;
 }
</script>
</head>
<body>
<!-- Navigation -->
    ...
    <div>
    	...
    	<h1> Choose Table for Update </h1>
    	<form action="PageManagerC" method="post">
			<input type="hidden" id="hidden" name="command" value="branch" />
			<c:set var="tablelists" value='${requestScope.tablelist}'/> 
	        <select id="tablePage" onchange="setHiddenVal()">
		        <c:forEach var="table" items="${tablelists}" >
		        	<option value="${table}">${table}</option>
				</c:forEach>
	        </select>
        	<input type="submit"  value="Go Update Table" />
        </form>
    </div>
