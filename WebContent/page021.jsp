...
<!-- Navigation -->
    ...
    <div>
    	<h1> Welcome to update Frequents Table </h1>	
    	<h4> Enter your name and your favorite branch here</h4>
		<form action="ManagerC" method="post">
		<input type="hidden" name="action" value="update"/>
		<input type="hidden" name="table" value="frequents"/>
	    <c:set var="branchlists" value='${requestScope.branchlist}'/> 
		<label class ="pad_top">Student ID:</label>
		<input type="text" name="id"/>
		<br>
		<div>
			<label>Branch:</label>
			<c:forEach var="branch" items="${branchlists }">
				<input type="radio" name="branchname" value="${branch}"> ${branch}
			</c:forEach>
		</div>
		<br>
	    <div>
